...
#' @param gene A character vector containing gene symbols. See \code{\link{dn_gene_ID}} for available gene symbols.
#' @param page.Size Number of records per page. For downloading data of single barcodes or genes the page.Size should be chosen small, i.e. 25, but usually not larger than 1000. # If there is an error "Error in mut.Exp[[page.Counter]]: subscript out of bounds", try another (usually smaller) page.Size.
...
#' @param page.Size Number of records per page. Usually max is 2000. For cancers with small number of patients use a small page.Size, e.g. page.Size = 25. # If there is an error "Error in mut.cohort[[page.Counter]]: subscript out of bounds", try a smaller page.Size.
#' @return data.frame of selected columns from the MAF generated by MutSig for the specified cohort.
#' @export
...
dn_mutation_cohort = function(cohort, page.Size, filename=NULL){
  ...
  cohort.clinical = dn_clinical_cohort(cohort)
  #  cohort.mutation = mclapply(cohort.clinical[,1], dn_mutation.Exp, cohort, "", page.Size, "Hugo_Symbol", mc.cores=20) # mc.cores < 32
  cohort.mutation = lapply(cohort.clinical[,1], dn_mutation.Exp, cohort, "Hugo_Symbol", page.Size, "")
 ...
 # for emergency
#   bc = barcode(cohort.mutation[,"Tumor_Sample_Barcode"])
#   return(data.frame(tcga_patient_barcode=bc, cohort.mutation))
