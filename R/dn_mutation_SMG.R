...
dn_mutation.SMG.Exp  = function(gene, rank, cohort, page.Size, sort_by, filename=NULL){
  ...
  while(all.Found == F){
    ...
    tmp = tryCatch(Analyses.Mutation.SMG(format = "csv", cohort = cohort, tool = "MutSig2CV", rank = rank, gene = gene, q = "",
                                          page = page.Counter, page_size = page.Size, sort_by = sort_by), error=function(e) NULL)
    
    # if( is.null(tmp)==TRUE || length(tmp)<1) { tmp = NULL; tmp = as.data.frame(tmp); break; }
